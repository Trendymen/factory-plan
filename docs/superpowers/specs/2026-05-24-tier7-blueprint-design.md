...
|---|---:|---:|---:|---:|
| Tier 6 基准 | 250 台 | 96 台（关闭其余 ~154 台 Power Switch）| ~2 GW | 26 mainNode |
| **当前 Tier 7**（已解锁）| 250 台（不变）| **96 台**（Mk5 升级，激活数仍为 T6 阶段）| **~2 GW** | 26 mainNode |
...
1. **物理建造 250 台机器**（按 Tier 7+ 满载尺寸配蓝图），但只**激活 96 台**（用 Power Switch 关闭其余，T7 当前阶段）
...
铝制外壳·复合铝板·超级计算机·无线电控制单元·冷却系统·涡轮马达·焊接模块化框架·叠加振荡器·时间水晶·神经量子处理器·F金三角

> 注：**F金锭** 不在最终产物列表里——它是 F金三角 的纯中间产物（1 锭 → 3 三角，构筑站配方），不作为建筑施工材料、没有 sink 积累价值、没有任何外部需求，故不计为 mainNode。原 a.data 把它当 mainNode 是误标。

### 当前阶段产能/需求摘要

- **机器总数（物理建造）**：250 台（按 Tier 7+ 满载预建）
- **机器总数（当前激活）**：96 台（按 26-plan 跑）
...
铁板·铁棒·电线·电缆·混凝土·铜板·钢梁·钢管·强化铁板·转子·模块化框架·钢筋混凝土梁·定子·电机·重型模块化框架·计算机·塑料·橡胶·急速电线·石英晶体·二氧化硅·**AI限制器**·**晶体振荡器**·**高速连接器**·**活性SAM物质**·**SAM物质波波动器**

### Tier 7-9 待激活的 mainNode（11 个，预留 Dim Depot 槽位）

铝制外壳·复合铝板·超级计算机·无线电控制单元·冷却系统·涡轮马达·焊接模块化框架·叠加振荡器·时间水晶·神经量子处理器·F金三角

> 注：**F金锭** 不在 mainNode 列表（纯中间产物，1 锭→3 三角，无建筑用途，不积累）。最终 26 + 11 = **37 mainNode**。
...
6. **副产品 重油残渣**（流体）：每个 BP9 实例**本地**用 1 台 refinery 跑 `petroleum-coke` 配方转固体油焦（每实例 ~47/min），**就地 sink**（每实例配 1 个 awesome-sink）。残渣本身是流体不能上 belt，AWESOME Sink 也不接受流体，必须就地转固体；同时油焦 234/min 上 B6 会让 B6 BP10→BP15 段达 69% Mk5（540/780），为 T8/T9 扩产保留余量，**不上总线**。详见"重油残渣处理"小节

### 可验证成功标准

- 96 台机器全部分配到 Mk2 蓝图，每台时钟符合方案 C 计算结果
...
| **C3 铜+电链** | 铜锭+钦金锭(BP6) → 铜板/电线/电缆(BP7) → 电路板+AI限制器(BP8) | 铜矿石 302 / 钦金锭 74 / 铜锭 302 / 电线 200（C3 内部部分消费）|
| **C4 油+橡塑** | 塑料/橡胶+油焦(BP9) | 原油 / 水（残渣 pipe 内部转油焦 → 就地 sink，不上总线）|
| **C5 MAM+混凝土+SAM** | 石英晶体/二氧化硅/急速电线/混凝土+AI限制器(BP10) → **活性SAM物质/SAM物质波波动器(BP11)** | 原始石英 / 石灰石 / SAM 矿（360/min） |
| **C6 装配+终端** | 转子/定子/电机(BP12) → 模块化框架/钢筋混凝土梁(BP13) → 重型模块化框架/计算机(BP14) → 晶体振荡器/高速连接器(BP15) | C6 内部装配链 |
...
| **B6** | caterium 锭（钦金锭）74 + 铜片 Copper Sheet 25 + 急速电线 210 + 石英晶体 18 | 最大段 228（BP10→BP15）| 29% | 可编程分流器（多 filter）|

#### Tier 7+ 预留 2 条 reserved belt

| Belt | 预留用途 | 启用阶段 |
|---|---|---|
| **B7** | 铝链中间品（铝锭 → 铝制外壳 → 复合铝板 → 散热器）| Tier 8 解锁后 |
...
| BP6 | 铜锭/钦金锭 (smelter) | 物理 **33**（T9 激活上限 31）| 5+2 = 7 | 26 | **拆 3 个 Mk2** (BP6a/BP6b/BP6c) |
| BP7 | 铜板/电线/电缆 (constructor) | 18+20+8 = **46** | 3+6+2 = 11 | 35 | **拆 3 个 Mk2** (BP7a/b/c) |
...
| BP11 | 活性SAM物质+SAM物质波波动器 | 2+1 = 3 | 2+1 = 3 | 0 | 单 Mk2，无扩容 |
| BP12 | 转子/定子/电机 (assembler) | 3+3+1 = 7 | 2+2+1 = 5 | 2 | 单 Mk2 |
| BP13 | 模框/包裹 (assembler) | 4+2 = 6 | 3+2 = 5 | 1 | 单 Mk2 |
| BP14 | HMF/计算机 (manufacturer) | 物理 **6**（1 HMF + 5 计算机）| 1+1 = 2 | 4 | **拆 3 个 Mk2** (BP14a/b/c，manufacturer 20×22m，1 Mk2 装 2 台)；T6 激活 2 |
...
- **输出**:
  - 钢梁 → 屋顶总线 B3（48/min 给 C6 BP13 钢筋混凝土梁）+ B5 终端（15/min mainNode）
  - 钢管 → 屋顶总线 B3（45 给 BP12 定子 + 40 给 BP14 HMF + 30 给 BP11 SAM物质波波动器 = 115/min 总）+ B5 终端（20/min mainNode）
- **屋顶总线接入**: 输出 B3（钢梁 48 + 钢管 115 = 163）+ 输出 B5（钢梁 15 + 钢管 20 = 35）

### BP6: 铜锭 + 钦金锭

- **集群**: C3
- **实例/物理**: **3 Mk2 (BP6a/b/c)**，物理 **33** smelter（T9 激活上限 31）；以下为 **T6 激活 7 台**（5 铜锭 + 2 钦金锭，全在 BP6a）
- **机器（T6 激活）**: 5 铜锭 smelter + 2 钦金锭 smelter = **7 台**
- **超频**: 铜锭 5 × 217.67% / 钦金锭 2 × 246.67%
- **产能**: 铜锭 326.5/min · 钦金锭（caterium 锭）74/min
- **机器层 (0-35m，按 §2 公约）**:
  - smelter 实测 **6×9×10m**（W×L×H，§1）；输入 back 在北，**北墙内缩 4m 留铜矿石/钦金矿石进料巷**。
  - 1F (0-10m): 4 smelter（3 铜锭 + 1 钦金锭）东西向一排（4 × 6m + 间隙 ≈ 27m，留侧墙净空），机器占 y=4-13m。
  - 2F (14-24m, 4m 地基 10-14): 3 smelter（2 铜锭 + 1 钦金锭）同样北侧进料巷。
- **屋顶总线层 (35-40m)**:
  - 自带 merger 把 钦金锭 注入 B6（74/min 给 BP10 急速电线）
- **输入**:
  - 铜矿石 326.5/min（**矿场直接喂**）
  - 钦金矿石 222/min（**矿场直接喂**，74 × 3 = 222 因为钦金锭配方是 3 ore → 1 ingot）
- **输出**:
  - 铜锭 326.5/min → 集群内部 BP7（**集群内部短 belt**）
  - 钦金锭 74/min → 屋顶总线 B6（→ BP10 给急速电线产线，74/min 全部）
...
- **集群**: C3
- **机器**: 3 铜板（铜片 Copper Sheet）+ 6 电线 + 2 电缆 constructor = **11 台**
- **超频**: 铜板 3 × 208.33% / 电线 6 × 223.89% / 电缆 2 × 169.17%
- **产能**: 铜板 62.5/min · 电线 403/min · 电缆 101.5/min
...
- **输入**:
  ...
  - 电线 203/min（BP7 内部回流给电缆，2 × 169.17% × 60 = 203 wire/min 输入）
- **输出**:
  - 铜板 → 集群内部 BP8 电路板（27.5/min）+ 屋顶总线 B6（25/min 给 BP10 AI 限制器）+ B5 终端（10/min mainNode）
  - 电线 → BP7 内部电缆（203/min）+ 屋顶总线 B3（120 给 BP12 定子 + 50 给 BP11 SAM物质波波动器 = 170/min）+ B5 终端（30/min mainNode）
  - 电缆 → 屋顶总线 B4（14 给 BP15 晶振 + 20 给 BP14 计算机 + 37.5 给 BP15 HSC = 71.5/min）+ B5 终端（30/min mainNode）
- **屋顶总线接入**: 输出 B3（电线 170）+ 输出 B4（电缆 71.5）+ 输出 B6（铜板 25）+ 输出 B5（铜板 10 + 电线 30 + 电缆 30 = 70）
...
- **输出**: 电路板 13.75/min → 屋顶总线 B4（10/min 给 BP14 计算机 + 3.75/min 给 BP15 高速连接器）
- **屋顶总线接入**: 输入 B4（塑料 55）+ 输出 B4（电路板 13.75）

> **AI 限制器去哪了？** 移到了 BP10（C5）。原因：AI 限制器吃 100/min 急速电线，急速电线在 BP10 产，把 AI 限制器放 BP10 内部直连可省一条总线 belt（如果放 BP8 反向跨集群运 100 急速电线很费事，bus 是单向的）。

### BP9: 塑料 + 橡胶

- **集群**: C4
- **机器**: 3 塑料 refinery + 1 橡胶 refinery + **1 coke refinery = 5 refinery**（每实例自包含残渣→油焦本地处理）+ 1 共享 awesome-sink
- **超频**: 塑料 3 × 191.67% / 橡胶 1 × 100% / coke 1 × 195%（2 shard，处理本实例 78/min 残渣）
- **产能**: 塑料 115/min · 橡胶 20/min · **油焦 ~234/min**（残渣 ~78/min 本地转化，就地 sink）
- **机器层 (0-35m，单层 1F)**:
...
- **屋顶总线层 (35-40m)**:
  ...
  - **本地 sink 油焦**（每实例配 1 awesome-sink，吃本实例 ~234/min 油焦，1 条 Mk5 入料 belt，234 << 780/min；sink 无超频按 belt 速率；不上 B6）
...
- **输出**:
  ...
  - 重油残渣 ~78/min → **本地 coke refinery 转油焦** ~234/min → 本地 awesome-sink（每实例 1 个）
- **屋顶总线接入**: 输出 B4（塑料 95）+ 输出 B5（塑料 20 + 橡胶 20 = 40）；**B6 不接入**（残渣/油焦不上总线）

> **重油残渣处理（最终方案）**：BP9 每实例**本地** 1 台 coke refinery 转油焦 + 1 个本地 awesome-sink；**残渣是流体不能上 belt**，油焦 234/min 上 B6 虽 Mk5 可承载（540/780 = 69%），但**主动为 T8/T9 扩产保留头部空间**，因此就地 sink。BP-TERM-B 不再有残渣/油焦 sink。
>
...
- **集群**: C5
- **机器**: 1 石英晶体 + 1 二氧化硅 + 3 急速电线 + 3 混凝土 constructor + 1 AI 限制器 assembler = **9 台**
- **超频**:
  - 石英晶体 1 × 180% / 二氧化硅 1 × 100% / 急速电线 3 × 205.56% / 混凝土 3 × 246.67%
  - AI 限制器 1 × 100%
- **产能**: 石英晶体 40.5/min · 二氧化硅 37.5/min · 急速电线 370/min · 混凝土 111/min · AI 限制器 5/min
- **机器层 (0-35m)**:
  - constructor 实测 8×10×8m，assembler 实测 10×15×8m。
  - 1F (0-8m，北墙内缩 4m 留进料巷)：8 constructor 分 **2 行**（每行 ≤4 台留侧向 belt 净空，不满宽零间隙）：第 1 行 1 石英晶体 + 1 二氧化硅 + 2 急速电线（4 台 × 8m + 间隙 ≈ 36m），第 2 行 1 急速电线 + 3 混凝土（4 台）+ 1 AI 限制器 assembler（10×15m，与 constructor 行错开放在南侧空档）。
  - 占地校验：8 constructor × 80m²（8×10）= 640m² + 1 assembler 150m²（10×15）= 790m²，1F 1600m² 装下（有富余留进料巷与 belt 折线净空）。
- **屋顶总线层 (35-40m)**:
  - 自带 smart splitter 从 B6 取 **caterium 锭（钦金锭）74/min**（→ 3 急速电线 constructor；**标准 quickwire 配方原料就是 caterium 锭**，由 BP6 的 caterium 锭产线供给，与钦金锭来源一致）
  - 自带 merger 注入 B6：急速电线 210/min（给 BP15 HSC）+ 石英晶体 18/min（给 BP15 晶振）
  - 自带 merger 注入 B4：混凝土 96/min（给 BP13 包裹）
  - 自带 merger 注入 B5：mainNode 量 = 石英晶体 22.5 + 二氧化硅 37.5 + 急速电线 60 + 混凝土 15 + AI 限制器 5 = 140/min
- **输入**:
  - 原始石英 78/min（**矿场喂**，40.5/min 给石英晶体 + 37.5/min 给二氧化硅）
  - **caterium 锭（钦金锭）74/min**（**屋顶总线 B6** ← BP6；急速电线 100% 消耗，是急速电线的标准配方原料）
  - 石灰石 333/min（**矿场喂**）
...
- **输出**:
  ...
  - 二氧化硅 37.5/min → 屋顶总线 B5（mainNode）
  - 急速电线 370/min → 集群内部 AI 限制器（100/min）+ B6（210/min 给 BP15 HSC）+ B5（60/min mainNode）
  - 混凝土 111/min → B4（96 给 BP13）+ B5（15 mainNode）
  - AI 限制器 5/min → B5（mainNode）
- **屋顶总线接入**: 输入 B6（caterium 锭 74 + 铜片 25 = 99）+ 输出 B6（急速电线 210 + 石英晶体 18 = 228）+ 输出 B4（混凝土 96）+ 输出 B5（140 mainNode）

> **铜片路由**：铜片 Copper Sheet（BP7/C3 产）→ BP10（C5），跨 C3→C4→C5。与 caterium 锭同向，统一走 B6：BP6 注入 caterium 锭、BP7 注入铜片（25/min），都流向 BP10；BP10 屋顶用智能分流器分别取 74 caterium 锭和 25 铜片。B6 之后段（BP10→BP15）只剩 急速电线 210 + 石英晶体 18 = 228/min。

### BP11: 重生 SAM + SAM 波动器（**新增，C5 末端**）

- **集群**: C5（紧贴 BP10 之后，C5 末端）
- **机器**: 2 活性SAM物质 constructor + 1 SAM物质波波动器 manufacturer = **3 台**
- **超频**: 活性SAM物质 2 × 150% / SAM物质波波动器 1 × 100%
- **产能**: 活性SAM物质 90/min · SAM物质波波动器 10/min
- **机器层 (0-35m)**:
  - 1F (0-12m): 1 SAM物质波波动器 manufacturer（实测 **20×22×12m**，**facing=south**：输入 front=南、输出 back=北）；机器占 y=4-26m，**北侧 ≥4m output lift 巷、南侧 row22-40 做 front 进料 manifold 平面**（电线/钢管/活性SAM物质 经南侧水平转弯相 → 进 front 口）。
  - 2F (16-28m, 4m 地基 12-16): 2 活性SAM物质 constructor（实测 8×10×8m，2 台 × 8m = 16m 宽 × 10m 长）
- **屋顶总线层 (35-40m)**:
  - 自带 smart splitter 从 B3 取 电线 50/min（→ SAM物质波波动器）+ 钢管 30/min（→ SAM物质波波动器）
  - 自带 lift-bot 把 活性SAM物质 90/min 从 2F 送到 1F SAM物质波波动器（其中 60 内部消耗 + 30 上 lift-top → B5）
  - 自带 merger 把 活性SAM物质 30/min + SAM物质波波动器 10/min 注入 B5（mainNode）
- **输入**:
  - SAM 矿石 360/min（**SAM 矿场直接喂**，recipe SAM×120 → 活性SAM物质×30，1 台 100% 用 120 SAM 出 30 活性SAM物质。Plan C 2 台 × 150% = 3 effective machines × 120 = 360 SAM/min ✓）
  - 电线 50/min（**屋顶总线 B3** ← BP7）
  - 钢管 30/min（**屋顶总线 B3** ← BP5）
- **输出**:
  - 活性SAM物质 → 集群内部消费给 SAM物质波波动器（60/min）+ 屋顶总线 B5 mainNode（30/min）
  - SAM物质波波动器 → 屋顶总线 B5 mainNode（10/min）
- **屋顶总线接入**: 输入 B3（电线 50 + 钢管 30 = 80）+ 输出 B5（活性SAM物质 30 + SAM物质波波动器 10 = 40）
...
- **集群**: C6
- **机器**: 3 模块化框架 + 2 钢筋混凝土梁 assembler = **5 台**
- **超频**: 模块化框架 3 × 200.0% / 钢筋混凝土梁 2 × 133.33%
- **产能**: 模块化框架 12/min · 钢筋混凝土梁 16/min
- **机器层 (0-35m，按 §2 公约）**:
...
- **输出**:
  ...
  - 钢筋混凝土梁 → 集群内部 BP14 HMF（10/min）+ 屋顶总线 B5（6 mainNode）
- **屋顶总线接入**: 输入 B2（强化铁板 18 + 铁棒 72 = 90）+ 输入 B3（钢梁 48）+ 输入 B4（混凝土 96）+ 输出 B5（8 mainNode）

### BP14: 重型模块化框架 + 计算机

- **集群**: C6
- **机器**: 1 HMF + 1 计算机 manufacturer = **2 台**
- **超频**: HMF 1 × 100% / 计算机 1 × 100%
- **产能**: HMF 2/min · 计算机 2.5/min
- **机器层 (0-35m)**:
  - manufacturer 实测 **20×22×12m**（W×L×H，§1）；**facing=south**（唯一输入在 front=南 4 口、输出 back=北 1 口；删除任何 facing=north / port reversed 措辞）。
  - 1F (0-12m): 1 HMF + 1 计算机 manufacturer 东西向并排（2 台 × 20m = 40m 宽 × 22m 长）。**facing=south 进料巷在南侧**（机器 back 北侧 y=0-? 留 ≥4m 给 output lift；front 输入在南侧 y≈22m 处接进料 manifold）。
...
- **屋顶总线层 (35-40m)**:
  ...
  - 自带 programmable splitter 从 B4 取 电路板（10）+ 电缆（20）+ 塑料（50）→ 计算机
  - 自带 merger 把 HMF + 计算机 mainNode 量注入 B5（4.5/min）
  - **output lift**：HMF/计算机成品从 manufacturer back（北）输出口经北侧 output lift 巷垂直上送屋顶（lift 集中专用 col，跨度 ≥23m）。
- **输入**（HMF）:
  - 模块化框架 10/min（**集群内部短 belt** ← BP13）
  - 钢筋混凝土梁 10/min（**集群内部短 belt** ← BP13）
  - 钢管 40/min（**屋顶总线 B3** ← BP5）
  - 螺丝 240/min（**屋顶总线 B2** ← BP3）
- **输入**（计算机）:
  - 电路板 10/min（**屋顶总线 B4** ← BP8）
  - 电缆 20/min（**屋顶总线 B4** ← BP7）
  - 塑料 50/min（**屋顶总线 B4** ← BP9）
- **输出**:
  - HMF 2/min → 屋顶总线 B5（mainNode）
  - 计算机 2.5/min → 屋顶总线 B5（mainNode）
- **屋顶总线接入**: 输入 B2（螺丝 240）+ 输入 B3（钢管 40）+ 输入 B4（电路板 10 + 电缆 20 + 塑料 50 = 80）+ 输出 B5（4.5 mainNode）
...
- **屋顶总线层 (35-40m)**:
  - 自带 smart splitter 从 B6 取 急速电线 210/min（→ HSC）+ 石英晶体 18/min（→ 晶振）
  - 自带 programmable splitter 从 B4 取 电缆 51.5/min（14 给晶振 + 37.5 给 HSC）+ 电路板 3.75/min（→ HSC）
  - 自带 smart splitter 从 B2 取 强化铁板 2.5/min（→ 晶振）
  - 自带 merger 注入 B5：晶振 1/min + HSC 3.75/min = 4.75 mainNode
- **输入**:
  - 强化铁板 2.5/min（**屋顶总线 B2** ← BP2）
  - 电缆 51.5/min（**屋顶总线 B4** ← BP7）
  - 电路板 3.75/min（**屋顶总线 B4** ← BP8）
  - 急速电线 210/min（**屋顶总线 B6** ← BP10）
  - 石英晶体 18/min（**屋顶总线 B6** ← BP10）
- **输出**:
  - 晶体振荡器 1/min → B5（mainNode）
  - 高速连接器 3.75/min → B5（mainNode）
- **屋顶总线接入**: 输入 B2（RIP 2.5）+ 输入 B4（电缆 51.5 + 电路板 3.75 = 55.25）+ 输入 B6（急速电线 210 + 石英晶体 18 = 228）+ 输出 B5（4.75 mainNode）
...
- **Tier 7+ 预留 11 个槽位**：未来铝制外壳/复合铝板/超级计算机/RCU/冷却系统/涡轮马达/焊接模块化框架/叠加振荡器/时间水晶/神经处理器/F金三角 解锁后接入。BP-TERM 实际部署 **27 个 Uploader 槽位 + 11 个预留 slot**，27 路分流器树最多可分 27 路（用 1→3→9→27 三级树）。Tier 7+ 扩容时把分流器树扩成 1→3→9→27→81 四级树（最多 81 路，覆盖 37 mainNode 充分有余）。
...
| 产物 | 37-plan 产能 | 主消费方向 |
|---|---:|---|
| 复合铝板 | 210/min | 散热器、叠加振荡器 |
| 铝制外壳 | 235/min | 无线电控制单元、焊接模块化框架 |
| F金三角 | 75/min | 神经量子处理器 |
| 时间水晶 | 21 | 神经量子处理器 |
| 冷却系统 | 13.5 | 涡轮马达 |
| 无线电控制单元 | 6.25 | 涡轮马达 |
...
| 当前机器数（Plan C）| **96 台** | Tier 9 全开后扩容到 ~285 台 |
...
4. **C5 集群末端预留 2 个 Mk2 蓝图位**：未来 Tier 9 加 1 个虚构链蓝图（F金锭+F金三角同蓝图，F金锭仅作为内部中间产物）+ 1 个时间水晶/暗物质链蓝图；C7 新集群预留位置给铝链
...
每个 BP9 实例**自包含**：
- 1 台 refinery 跑 `petroleum-coke` 配方（40 残渣 → 120 油焦），处理本实例 78 残渣（@195% ≈ 78 残渣处理能力，2 shard）
- 1 台 awesome-sink（**无超频、无内在吞吐上限，按入料 belt 速率吃料**；本实例油焦 ~234/min 用 1 条 Mk5 belt 入料即可，234 << Mk5 780/min）
...
- 234 油焦/min 上 B6 → BP10→BP15 段流量 = 306 + 234 = 540/min = **69% Mk5 容量**（物理上可承载）
- **主动保留**：T8/T9 扩产时 BP10→BP15 段流量可能达到 570-590/min（急速电线 × 7 实例等），Mk5 额定 780/min 仍有余量，但如果油焦也上来将减少 T8/T9 扩容头部空间
...
| `petroleum-coke`（refinery，**本设计默认**）| 40 残渣 → 120 油焦 | 1 台 refinery × ~195%（约 250% 余量充足）|
...
| BP6a/b/c | 铜锭/钦金锭 | 5+2 | 14+4 | 20+6 | 23+8 | 拆 3 个 Mk2；钦金锭因 RCU/HSC uplift 显著 |
| BP7a/b/c | 铜板/电线/电缆 | 3+6+2 | 8+12+4 | 14+16+6 | 18+20+8 | 拆 3 个 Mk2，电线最紧张 |
| BP8 | 电路板 | 1 | 4 | 6 | 8 | 计算机 uplift 11x，电路板大头 |
| BP9a-g | 塑料/橡胶/coke | 3+1+1 | 11+1+3 | 18+2+5 | 23+2+7 | T6 BP9a 单实例满载（5 台）；T7 3 实例；T8 5 实例；T9 **7 实例**（BP9a-g，全部 Plan C 250%）|
| BP10a/b | 石英/二氧化硅/急速电线/混凝土/AI 限制器 | 1+1+3+3+1 | 2+3+5+4+1 | 3+5+8+5+2 | 4+7+10+5+2 | 拆 2 个 Mk2，二氧化硅最高 18x |
| BP11 | 活性SAM物质/SAM物质波波动器 | 2+1 | 2+1 | 2+1 | 2+1 | uplift 1.0-2.2x，无变化 |
| BP12 | 转子/定子/电机 | 2+2+1 | 3+3+1 | 3+3+1 | 3+3+1 | T7 起满载 |
| BP13 | 模框/钢筋混凝土梁 | 3+2 | 3+2 | 4+2 | 4+2 | 装配类，1.5-1.6x |
| BP14a/b/c | HMF/计算机 | 1+1 | 1+3 | 1+4 | 1+5 | 拆 3 个 Mk2；计算机 11x（5 manufacturer 满载） |
...
| **B4** | 塑料/混凝土/电路板/电缆 → C6 | 塑料 ~10x（✓（Mk5 已升））| 电路板/电缆 倍增 | — |
...
| **B9**（新增）| — | — | **粒子加速中间品 + 涡轮马达/融合模块/冷却系统**（C8→BP-TERM/C9）| — |
| **B10**（新增）| — | — | — | **量子编码 + 时间水晶/暗物质/虚构链**（C9→BP-TERM）|
...
| **BP-TERM-B** | 13 mainNode（后半批：装配/MAM/SAM）| +1 铝制外壳 +1 RCU +1 超级计算机 | +1 涡轮马达 +1 融合模块 +1 冷却系统 | +1 神经量子处理器 +1 叠加振荡器 +1 F金三角 |
...
- **输入**:
  - 铝土矿 ~360/min（**矿场直接喂**）
  - 水 ~360/min（**水泵管道**）
  - 油焦 ~210/min（**屋顶总线 B7** ← BP9b residual-fuel 旁挂的 petroleum-coke 蓝图，或新增专用蓝图）
- **输出**:
  - 氧化铝溶液 → 集群内部 → BP17 铝锭
  - 碎铝渣 → 集群内部 → BP17 铝锭 + BP18 复合铝板
...
- **集群**: C7
- **机器**: 8 铝锭 foundry + 4 铝制外壳 constructor = **12 台**
- **机器层**: 1F 8 foundry（4 行 × 2 = 32m 宽 × 18m 长）+ 2F 4 constructor
- **输入**:
  - 碎铝渣 + 硅 + 煤（从 C2 反向通过 B7 取，或本地小煤矿场）
- **输出**:
  - 铝锭 → 集群内部 BP18
  - 铝制外壳 → B7（→ BP19 RCU + 融合模块 + B5a 终端 60/min mainNode）

#### BP18: 复合铝板 + 散热器

- **集群**: C7
- **机器**: 6 复合铝板 foundry + 4 散热器 assembler = **10 台**
...
- **输入**:
  ...
  - 铝制外壳（**集群内部** ← BP17）
- **输出**:
  - 复合铝板 → B7（→ BP19/BP21 + B5 终端 30/min）
...
#### BP19: 涡轮马达 + 冷却系统 + 焊接模块化框架

- **集群**: C8
- **机器**: 1 涡轮马达 manufacturer + 2 冷却系统 blender + 2 焊接模块化框架 manufacturer = **5 台大型机**
- **机器层**: blender 19m × 16m × 16m，manufacturer 20m × 22m × 12m → 拆 2 个 Mk2 (BP19a 涡轮马达+冷却 / BP19b 融合模块)
- **关键输入**: 铝制外壳/复合铝板（B7）+ 重型模块化框架（B5 反向取，或集群内 BP14 重型框架旁挂）+ 液氮（专用气体管道）+ 重生 SAM（**屋顶 B5/B7 取，BP11 上行**）
- **输出**: 涡轮马达/冷却系统/焊接模块化框架 → B5 mainNode 终端
...
- **输入**:
  - RCU: 铝制外壳（B7）+ 电路板（B4 反向，或集群内）+ 转子（B5 反向，或集群内）
  - 超级计算机: 计算机（**集群内** ← BP14）+ AI 限制器（B5 反向，或 BP10 上行）+ HSC（B5 反向，或 BP15 上行）+ 塑料（B4）
- **输出**: RCU + 超级计算机 → B5 mainNode 终端

> **C8 反向路由问题**：粒子链消耗大量上游产物（重型框架、计算机、AI 限制器、HSC），都在 C6/C5 产。**实施时决策**：
...
- **集群**: C9
- **机器**: 2 转化站（converter，16×16×16m）+ 1 粒子加速站（38×24×38m，时间水晶配方）= **3 台**
- **输入**:
  - 钻石（**集群内** 或独立 converter 蓝图）
  - 暗物质晶体（**集群内** ← BP23）
- **输出**:
  - 时间水晶 → BP21 + B10 → BP-TERM mainNode

#### BP23 (Mk3): 暗物质 + 激发光子

- **集群**: C9
- **机器**: 2 转化站（暗物质晶体配方）+ 1 转化站（激发态光子物质配方）= **3 台**
- **输入**: SAM 矿石（**集群内** ← BP11 反向，或独立 SAM 矿场）+ 重生 SAM（**集群内**）
- **输出**: 暗物质晶体/激发光子 → 集群内 BP21/BP22

#### BP24 (Mk3): 虚构链（F金锭 + F金三角）

- **集群**: C9 末端
- **机器**: 1 粒子加速站（F金锭配方）+ 1 量子编码站或粒子加速站（F金三角配方）= **2 台**
- **输入**: 暗物质晶体 + 时间水晶 + 神经量子处理器 等（全部集群内）
- **输出**: F金三角 → B10 → BP-TERM mainNode（F金锭仅集群内中间产物，不上 B10）
...
2. ☐ 物理建造 C7 铝链（BP16a/b/c 氧化铝 refinery + BP17 铝锭/铝制外壳 + BP18 复合铝板/散热器）
3. ☐ 启用屋顶 B7（铝中间品）+ B8（超级计算机/RCU 上行）
4. ☐ BP-TERM-A/B 启用 6 个新 Uploader（铝制外壳/复合铝板/RCU/超级计算机/散热器/硅作为副产品 sink）
5. ☐ 物理建造 C8 粒子集群（BP19a/b 涡轮马达+冷却+融合模块）
6. ☐ 启用屋顶 B9（粒子链中间品）
7. ☐ 接氮气矿场（液氮管线）
8. ☐ BP-TERM-A/B 启用 3 个新 Uploader（涡轮马达/融合模块/冷却系统）
9. ☐ B3/B4/B6 已是 Mk5，流量翻倍仍充裕；B7/B8 此时已启用铝链/RCU 上行

#### Tier 9 解锁后（约 +20 台，+10 GW）

1. ☐ **解锁 Mk3 蓝图设计器**（与 Quantum Encoding milestone 同步）
2. ☐ 物理建造 C9 量子集群（BP21a/b 量子编码 + BP22 时间水晶/转化 + BP23 暗物质 + BP24 虚构链）
3. ☐ 启用屋顶 B10（量子链中间品）
4. ☐ BP-TERM-A/B 启用最后 5 个新 Uploader（神经处理器/叠加振荡器/F金三角/时间水晶 + 1 备用）；或启用 BP-TERM-C
...
| **计算机/HSC manufacturer T9 满载需要 5+2 + 4 晶振 = 多台** | BP14a/b/c 已物理建造 6 个 manufacturer 槽位（5 计算机 + 1 HMF），BP15a/b/c 物理建造 6 个槽位（4 晶振 + 2 HSC）|
...
| **Tier 7+ 新增** | | | | |
| 20 | BP16a/b/c（氧化铝/碎铝渣 refinery）| 复制 | 3 | C7 |
| 21 | BP17（铝锭/铝制外壳）| 唯一 | 1 | C7 |
| 22 | BP18（复合铝板/散热器）| 唯一 | 1 | C7 |
| 23 | BP19a（涡轮马达/冷却系统）| 唯一 | 1 | C8 |
...
- 完成 MAM Caterium 研究链（解锁 钦金锭/急速电线/AI 限制器/HSC）
...
7. **当前激活**：钢锭 5 + 钢梁 2 + 钢管 3 + 铜锭 5 + 钦金锭 2 + 铜板 3 + 电线 6 + 电缆 2 + 电路板 1 = 29 台
8. 矿场接入（铁矿石 + 煤 + 铜矿石 + 钦金矿石），按 Tier 7+ 流量预留

### 阶段 3：油 + MAM（C4 + C5）

9. **物理建造**：BP9a-g（**35 refinery 分七 Mk2，每个蓝图 5 台 = 3 塑料 + 1 橡胶 + 1 coke**，T6 即全部建好）+ BP10a/b（28 机器分两 Mk2）+ BP11（3 机器单 Mk2）
10. **当前激活**：塑料 3 + 橡胶 1 + 石英晶体 1 + 二氧化硅 1 + 急速电线 3 + 混凝土 3 + AI 限制器 1 + 活性SAM物质 2 + SAM物质波波动器 1 = 16 台
...
13. **当前激活**：转子 2 + 定子 2 + 电机 1 + 模框 3 + 包裹 2 + HMF 1 + 计算机 1 + 晶振 1 + HSC 1 = 14 台
...
| Tier 8 Particle / Leading-edge | 启用更多闲置机器 + 加粒子加速 + 混料站 + 涡轮马达 + 散热器 | C8 粒子加速集群（2-3 Mk2）| +12 | +5 GW |
| Tier 9 Matter Conversion / Quantum Encoding | 启用所有闲置 + 加转化站 + 量子编码站 + 虚构链 + 时间水晶链 | C9 量子编码集群（3-4 Mk2）| +20 | +10 GW |
...
| 钦金矿石 | 222 | **~600** | 3-4 Mk3 矿机（注意：很多地图铜金矿稀缺）|
| 煤矿 | 410 | **~1,500** | 6-8 Mk3 矿机 |
| 石灰石 | 333 | **~700** | 3-4 Mk3 矿机 |
| 原始石英 | 78 | **~600** | 3 Mk3 矿机（石英是 Tier 7+ 大头：晶振、虚构链、激发态光子物质都吃石英）|
...
| 氮气 | 0 | **~300** | Tier 7+ 才用（冷却系统/焊接模块化框架），先不接 |
...
至于其他大型建筑：
- **粒子加速站 实测 38×24×38m（高 38m，§1）**：底面 38×24 能塞进 Mk2 的 40×40，但**高 38m 会穿过 Mk2 屋顶总线层（35-40m）**——机器顶面 38m 撞进 35-40m belt 区。**因此粒子加速站也须放 Mk3 蓝图**（48m 净高，机器 38m + 屋顶 belt 留在 40m 以上才不冲突），不能放 Mk2。
- 转化站 16×16×16m（fits Mk2）、混料站 blender 19×16×16m（fits Mk2）—— 高度 16m，可装 Mk2。
