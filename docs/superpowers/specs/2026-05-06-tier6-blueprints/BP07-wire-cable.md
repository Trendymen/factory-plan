...
> 总产能验证: 3 × 20.83 = 62.5 铜板 ✓ | 6 × 67.17 = 403 电线 ✓ | 2 × 50.75 = 101.5 电缆 ✓<br>
...
| 输入 | 电线 | 203（内部回流，铜板/电线先做出来再喂给电缆）| 蓝图内部 lift |
...
| 输出 | 电缆 → B4 (BP14 + BP15) | 71.5 | 屋顶 merger → B4 |
| 输出 | 电缆 → B5 终端 | 30 | 屋顶 merger → B5 |
...
- 4 个 merger（B3=电线、B4=电缆、B5=mainNode 三合一、B6=铜板）
...
> 电线内部回流（自给电缆配方）**仅在同蓝图内部 lift**，不跨实例。<br>
...
- [ ] 三种产物 row belt 独立（铜板/电线/电缆）并接到全部 46 台 in-0/front（不只是 T6 通电的 11 台）
- [ ] 3 实例 × 4 = 12 个 Power Switch 一次建好；T6 仅 BP7a Network A + B 局部合上，其余全断
- [ ] T6 仅 11 台插了 shard；其余 35 台物理就位但 shard 槽空
- [ ] 4 个屋顶 merger filter 正确（B3=电线、B4=电缆、B5=mainNode 三种合、B6=铜板）
