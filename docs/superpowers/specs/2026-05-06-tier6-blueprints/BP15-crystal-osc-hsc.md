...
|---|---:|---:|---:|---:|
| crystal-oscillator (manufacturer) | 1 | **100.0%** | 1.0 | 0 |
| high-speed-connector (manufacturer) | 1 | **100.0%** | 3.75 | 0 |
| **合计 (T6)** | 2 | — | — | **0** |

> 总产能验证: 1 × 1.0 = 1 晶振 ✓ | 1 × 3.75 = 3.75 HSC ✓
> T6 阶段 2 台都 100% 不超频。T7+ 启用更多时各台 100% 即可（晶振 4 台 + HSC 2 台 = T9 满载 9 + 7.5 = 实际产能远超 mainNode 9.12 + 18.38）。
...
BP15a/b 在机器层**无跨实例短 belt**（晶振和 HSC 两条产线独立，进料全从屋顶取）。

蓝图侧墙集群内 mount（机器层）：**无**。BP15 仅有屋顶 6 belt mount。

> BP15a 跑晶振、BP15b 跑 HSC，物料完全独立，通过屋顶 B2/B4/B6 各自取料 → 通过屋顶 B5 各自输出 mainNode。
