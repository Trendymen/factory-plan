...
| 输入 | 电缆 | 14 / 56 | **屋顶 B4** ← BP7 → splitter |
| 输入 | 石英晶体 | 18 / 72 | **屋顶 B6** ← BP10 → splitter |
| **HSC 输入** | | | |
| 输入 | 急速电线 | 210 / 420 | **屋顶 B6** ← BP10 → splitter |
| 输入 | 电缆 | 37.5 / 75 | **屋顶 B4** ← BP7 → splitter |
...
**屋顶总线接入**:
- 取自 B2 (强化铁板 T9 10)、B4 (电缆 T9 131 + 电路板 T9 7.5)、B6 (石英 T9 72 + 急速电线 T9 420 = 492)
...
B6 流入 BP15 共 T6 228 → T9 492（石英 72 + 急速电线 420），是 B6 最大段。
- BP10 注入 B6: 钦金锭 74 + 铜板 25 → 99，被 BP10 自身取走（fix：BP10 取 99 后 = 0）
- BP10 注入 B6: 石英 72 + 急速电线 420 = 492（**T9 新源**）+ 重油残渣 78（来自 BP9）= 570
