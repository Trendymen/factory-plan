...
- **机器**: **6 manufacturer 一次物理建造到位**（BP14a 1F HMF + 2F 计算机 = 2 台；BP14b 1F+2F 各 1 计算机 = 2 台；BP14c 1F+2F 各 1 计算机 = 2 台）
- **激活时间线**（仅翻 Power Switch + 插 Power Shard，不动结构）:
  - T6 → 2 台通电（BP14a 1F HMF + BP14b 1F 计算机）
  - T7 → 4 台通电（+BP14a 2F 计算机 + BP14c 1F 计算机）
  - T8 → 5 台通电（+BP14b 2F 计算机）
  - T9 → 6 台通电（+BP14c 2F 计算机，满载）
- **产能**: T6 HMF 2/min + 计算机 2.5/min / T9 HMF 2/min + 计算机 12.5/min
...
| 输入 | 钢筋混凝土梁 | 10 / 10 | **集群内部** ← BP13 (h=8m) |
| 输入 | 钢管 | 40 / 40 | **屋顶 B3** ← BP5 → smart splitter |
| 输入 | 螺丝 | 240 / 240 | **屋顶 B2** ← BP3 → smart splitter |
| **计算机 输入**（manufacturer 3 槽 + 1 空）| | | |
| 输入 | 电路板 | 10 / 50 | **屋顶 B4** ← BP8 → splitter |
| 输入 | 电缆 | 20 / 100 | **屋顶 B4** ← BP7 → splitter |
| 输入 | 塑料 | 50 / 250 | **屋顶 B4** ← BP9 → splitter |
| **输出** | | | |
| 输出 | HMF → B5 终端 | 2 / 2 | 屋顶 merger → B5 |
| 输出 | 计算机 → B5 终端 | 2.5 / 12.5 | 屋顶 merger → B5 |

**屋顶总线接入**:
- 取自 B2 (螺丝 240)、B3 (钢管 40)、B4 (电路板 50 + 电缆 100 + 塑料 250 = 400 T9 上限)
- 注入 B5 (HMF 2 + 计算机 12.5 = 14.5 mainNode T9)
...
| **BP14a** | 2F 计算机 manufacturer | 1 | 0 |
| **BP14b** | 1F 计算机 manufacturer | 1 | **1** |
| **BP14b** | 2F 计算机 manufacturer | 1 | 0 |
| **BP14c** | 1F 计算机 manufacturer | 1 | 0 |
| **BP14c** | 2F 计算机 manufacturer | 1 | 0 |
| **小计** | | **6** | **2** |
...
### BP14a 2F (16-28m): 1 计算机 manufacturer — T6 Power Switch 关
...
- 1 台计算机 manufacturer **物理建造完整**，T6 阶段 Power Switch **关**
- Belt manifold + lift 一次性接到 3 槽 in-0/1/2（in-3 空，计算机配方无第 4 物料）
- 通电节奏：T7 翻 Network B Switch ON

### BP14b 1F (0-12m): 1 计算机 manufacturer — T6 通电
...
- 1 台计算机 manufacturer，**T6 通电**（Network A）

### BP14b 2F (16-28m): 1 计算机 manufacturer — T6 Power Switch 关
...
- 1 台计算机 manufacturer **物理建造完整**，T6 Power Switch **关** → T8 翻 ON（Network C）

### BP14c 1F (0-12m): 1 计算机 manufacturer — T6 Power Switch 关
...
- 1 台计算机 manufacturer **物理建造完整**，T6 Power Switch **关** → T7 翻 ON（Network B）

### BP14c 2F (16-28m): 1 计算机 manufacturer — T6 Power Switch 关
...
- 1 台计算机 manufacturer **物理建造完整**，T6 Power Switch **关** → T9 翻 ON（Network D，满载）
...
- BP14b/c 取 B4 (电路板 + 电缆 + 塑料，按通电台数分配)
...
| 网 | 范围 | 数量 | T6 状态 | 升级触发 |
|---|---|---:|---|---|
| Network A | BP14a 1F HMF + BP14b 1F 计算机 | 2 | **ON** | — |
| Network B | BP14a 2F 计算机 + BP14c 1F 计算机 | 2 | OFF | T7 翻 ON |
| Network C | BP14b 2F 计算机 | 1 | OFF | T8 翻 ON |
| Network D | BP14c 2F 计算机 | 1 | OFF | T9 翻 ON |
...
5. **BP14a 2F (16-28m)**：放 1 台计算机 manufacturer（同布局，facing=north）
6. **BP14a 2F belt**:
   - 电路板 + 电缆 + 塑料：从 1F B4 splitter lift-up → manifold → in-0/1/2（in-3 空）
   - out-0 → lift-out-top → 屋顶 merger 注 B5
7. **BP14b/c 类似**：1F+2F 各 1 台计算机 manufacturer，4 路输入全部来自 B4（screw/pipe 不需要）
...
10. **Power Shard（T6 阶段）**：**0 shard**（计算机/HMF 100% 即可，无需超频）；shard 槽全部留空

## Tier 7+ 启用流程（仅翻 Switch + 插 shard，不动结构 / 不动 belt）

| Tier | 操作 | 通电总数 | 计算机产能 |
|---|---|---:|---:|
| T7 | 翻 Network B Switch ON → BP14a 2F + BP14c 1F 各启计算机（100% 无 shard）| 4 | 7.5 |
| T8 | 翻 Network C Switch ON → BP14b 2F 启计算机（100% 无 shard）| 5 | 10.0 |
| T9 | 翻 Network D Switch ON → BP14c 2F 启计算机（100% 无 shard），**计算机满载 5×2.5=12.5** | 6 | 12.5 |

> T9 时 B4 来料升级：电路板 50 / 电缆 100 / 塑料 250 — 屋顶 B4 程序分流器流量上限需按 Mk5 (780/min) 预留。

## 多实例侧墙续接

BP14a/b/c 在机器层**无跨实例短 belt**（HMF 和计算机两条产线独立）。所有进料从屋顶 B2/B3/B4 取，所有出料注 B5。
...
| h=8m row=3.5 | 钢筋混凝土梁 | **Inlet** ← BP13 | 悬空 | 不需要 |

> BP14a 紧贴 BP13 右侧，BP14b 紧贴 BP14a 右侧，BP14c 紧贴 BP14b 右侧。模框/包裹梁仅供 BP14a 的 HMF 消耗（10+10），BP14b/c 跑计算机不需要这两路。
