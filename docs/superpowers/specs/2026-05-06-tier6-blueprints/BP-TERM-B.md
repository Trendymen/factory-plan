...
- **激活时间线**（仅翻 Power Switch + 插 Power Shard，不动结构）:
  ...
  - T7 → 翻 Network B Switch ON → 铝制外壳 / RCU / 超级计算机 3 个 Uploader 通电
  - T8 → 翻 Network C Switch ON → 涡轮马达 / 融合模块 / 冷却系统 3 个通电
  - T9 → 翻 Network D Switch ON → 神经处理器 / 叠加振荡器 / F金三角 3 个通电（满 22 Uploader）
...
> 油焦 234/min（BP9 副产）已在 BP9 内部就地 sink，**不进 BP-TERM-B**。
> Tier 7+ 时二氧化硅 + 急速电线 流量极大，需要拆为 2-3 个 Uploader 并联（每个吃 < 780 Mk5 容量）。
...
| 2F | 16-32m | **9 个 T7+ Uploader 槽位物理建好 + smart splitter + belt + lift 全部接好**（铝制外壳/RCU/超级计算机/涡轮马达/融合模块/冷却系统/神经处理器/叠加振荡器/F金三角）— **T6 Power Switch 关（Network B/C/D 待 T7+ 渐次启用）**，仅 sink 占用区之外 |
...
| 16 | 钢筋混凝土梁 | encased-industrial-beam |
| 17 | 重型模块化框架 | heavy-modular-frame |
| 18 | 计算机 | computer |
...
5. **B6 末端**: 直通悬空（无 splitter 无 sink，油焦已在 BP9 处理）
...
| Network B | 2F 铝制外壳 / RCU / 超级计算机 Uploader | 3 | OFF | T7 翻 ON |
| Network C | 2F 涡轮马达 / 融合模块 / 冷却系统 Uploader | 3 | OFF | T8 翻 ON |
| Network D | 2F 神经处理器 / 叠加振荡器 / F金三角 Uploader | 3 | OFF | T9 翻 ON |
...
|---|---|---:|
| T7 | 翻 Network B Switch ON → 铝制外壳 / RCU / 超级计算机 3 个 Uploader 通电（如流量大需要超频，则插 shard）| 16 |
| T8 | 翻 Network C Switch ON → 涡轮马达 / 融合模块 / 冷却系统 3 个通电 | 19 |
| T9 | 翻 Network D Switch ON → 神经处理器 / 叠加振荡器 / F金三角 3 个通电；二氧化硅 / 急速电线高流量 Uploader 按需拆为 2-3 个并联，所有总线 belt 升 Mk5/Mk6 | 22 |

T9 总数 BP-TERM-A 16 + BP-TERM-B 22 = 38（含 1 个备用 slot）= **37 mainNode**。油焦 sink 在 BP9 处理，不计入。
