...
当前 26-mainNode 方案的产量是按"单台 100% 满载"基准设的。但**Tier 9 全开后**（神经量子处理器/超级计算机/虚构锭/虚构三角/铝链/无线电控制单元/涡轮电机/冷却系统/融合模块框架/铝壳/铝包铝板/叠加振荡器/时间晶体 这 12 个目前锁住的 mainNode），上游产物会有**远超当前的内部消耗**。
...
### 12 个 Tier 7+ 锁定 mainNode 的目标产能（参考）
...
4. **C5 集群末端预留 2 个 Mk2 蓝图位**：未来 Tier 9 加 BP15a（虚构锭）+ BP15b（虚构三角）；C7 新集群预留位置给铝链
