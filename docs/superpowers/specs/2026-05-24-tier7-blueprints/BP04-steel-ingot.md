# BP4 钢锭 (C2)

## 概要

- **集群**: C2 钢系（C1 之后第 4 个蓝图）
- **规格**: Mk2 单实例
- **机器**: **6 foundry 一次物理建造到位**（foundry 8×9×9m，2 输入在 back、1 输出在 front；steel-ingot 配方：3 铁矿石 + 3 煤 → 3 钢锭/min）
- **激活时间线**（仅翻 Power Switch + 插 Power Shard，不动结构）:
  - T6 → 5 台通电
  - T7 → 5 台通电（不变）
  - T8 → 6 台通电
  - T9 → 6 台通电（满载 239%，645/min ÷ 6 ÷ 45/min 基础 ≈ 238.6%，取整 239%）
- **产能**: T6 455 /min 钢锭

> **核心设计原则**：6 台 foundry T6 阶段就全部摆好 + belt/manifold/电网/Power Switch 全部接好。后续升 Tier 只翻 Switch + 插 shard，不动结构、不重新拉 belt。

## 极简示意图

```floorstack
# BP04 钢锭 (C2) · Mk2 40×40m · 自下而上
1F (0-9m) | 铸造站×3（钢锭） | 铁矿石·煤 | 钢锭 → 钢梁管线(BP5) | ↑2F:铁矿石·煤
2F (13-22m) | 铸造站×3（钢锭） | 铁矿石·煤 | 钢锭 → 钢梁管线(BP5) |
屋顶 (35-40m) | （无生产） | 原料带路过本层，不取料 | — |
```

> BP04 单实例，双进料（铁矿石+煤）各 1 条 Mk5；1F+2F 钢锭合流后送 BP5；屋顶仅原料带穿行，本层不取料。

## 超频 & 碎片

| 机器（配方） | 超频 | 每台碎片 |
|---|---|---|
| 铸造站（钢锭） | 202% | 3 |

**分阶段：T6 → 5 台 @202%（15 碎片）· T7 → 5 台（不变）· T8 → 全 6 台 @202%（18 碎片）· T9 → 全 6 台 @239%（锁定 645/min，18 碎片）**
