...
- **激活时间线**（仅翻 Power Switch + 按需给位面存储站上传器插 Power Shard，**AWESOME回收器不超频**，不动结构）:
  ...
  - T7 → 翻 Network B Switch ON → 铝壳 / RCU / 超级计算机 3 个位面存储站上传器通电
  - T8 → 翻 Network C Switch ON → 涡轮电机 / 融合模块 / 冷却系统 3 个通电
  - T9 → 翻 Network D Switch ON → 神经处理器 / 叠加振荡器 / 虚构三角 3 个通电（满 22 位面存储站上传器）
...
>
> **例外**：需要并联位面存储站上传器 / 拆 belt 的高流量路（如 T9 硅土 690、急速电线 1384），其全部并联线必须在所属蓝图 T6 阶段一次预建；本蓝图无富余净空，故这两路并联扩容移交 BP-TERM-C（详见下文）。
...
无超频：位面存储站上传器 / AWESOME回收器均不超频、不插碎片；T6 通电 14 台（1F 12 台 U14-U25 + 2F U26 + 共享AWESOME回收器），T7+ 逐步启用 2F 预留槽位（铝壳/RCU/超级计算机 → 涡轮电机/融合模块/冷却系统 → 神经处理器/叠加振荡器/虚构三角，每阶 3 台）。
