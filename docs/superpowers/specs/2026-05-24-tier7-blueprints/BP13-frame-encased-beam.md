...
1F (0-12m) | 组装机×4（模块化框架，T6 通电3） | 强化铁板 18·铁棒 72 | 模块化框架 12/min（10→HMF·电脑(BP14)·2→外运） |
2F (16-24m) | 组装机×2（包裹工业梁） | 钢梁 48·混凝土 96 | 包裹工业梁 16/min（10→HMF·电脑(BP14)·6→外运） |
屋顶 (35-40m) | 汇料台 | 强化铁板·铁棒·钢梁·混凝土 | 模块化框架 2·包裹工业梁 6 → 外运 |
```

> 两层独立生产（模块化框架/包裹工业梁各一层），各自出料后 10/min 走集群内短传送带直送 HMF·电脑(BP14)，余量经屋顶汇料台送往下游；T8 翻开第 4 台框架机时需同步调整屋顶可编程分流器配置。
